...
def set_secret(service_client, arn, token):
    ...
    if not conn:
        logger.error("setSecret: Unable to log into database with previous, current, or pending secret of secret arn %s" % sanitize_input_str(arn))
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info("setSecret: Successfully set password for user %s in PostgreSQL DB for secret arn %s." % (pending_dict['username'], sanitize_input_str(arn)))
    finally:
        conn.close()
